...
resource "aws_vpc_security_group_ingress_rule" "alb_https" {
  for_each = local.https_configured ? toset(var.allowed_ingress_cidrs) : toset([])
...
resource "aws_security_group" "ecs" {
  ...
  description = "Fargate tasks; no public ingress"
  vpc_id      = aws_vpc.main.id
...
resource "aws_vpc_security_group_ingress_rule" "ecs_from_alb" {
  ...
  description                  = "Only the ALB may reach the container port"
...
resource "aws_vpc_security_group_egress_rule" "ecs_https" {
  security_group_id = aws_security_group.ecs.id
  description       = "HTTPS to VPC endpoints and (if NAT is enabled) AWS APIs"
  cidr_ipv4         = "0.0.0.0/0"
  from_port         = 443
  to_port           = 443
  ip_protocol       = "tcp"
